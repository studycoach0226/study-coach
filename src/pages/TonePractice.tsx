...
export default function TonePractice() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log(`[DEBUG] Initial local lookup count (Tone): ${localPairs.length}`);
      ...
      console.error('[DEBUG] Failed to load initial data from local DB (Tone):', err);
    }
    ...
    const loadData = async () => {
      try {
        ...
        console.log(`[DIAGNOSTIC] TonePractice raw cloudDocs:`, cloudDocs);

        const cloudPairs = cloudDocs.map(doc => mapFirestoreToLocal(doc));
        console.log(`[DIAGNOSTIC] TonePractice cloudPairs:`, cloudPairs.map(p => ({
          id: p.item.id,
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '2rem auto' }}>
      ...
      {viewMode === 'overview' ? (
        ...
        {/* AI Voice Preference Selector - Hidden from UI */}
        {false && (
        ...
        {/* Green Curve Pipeline Selector - Hidden from UI */}
        {false && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', marginBottom: '2rem', padding: '0.5rem 1rem', background: '#f1f5f9', borderRadius: '20px' }}>
